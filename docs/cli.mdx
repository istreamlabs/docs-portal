...
iStreamPlanet's APIs implement OpenAPI 3 descriptions which are publicly linked via [RFC 8631](https://tools.ietf.org/html/rfc8631) `service-desc` link relation headers and auto-discovery for CLI configuration via `x-cli-config` extensions. This means you can use any client which understands these features to interact with our APIs.
...
? Organization [? for help] YOUR_VALUE_HERE
